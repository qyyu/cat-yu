...
var spritesmith = require('gulp.spritesmith');//合并图片
...
gulp.task('sprite', function () {
    var spriteData = gulp.src('./src/temp/*').pipe(spritesmith({
        imgName: 'sprite.png',
        cssName: 'sprite.css',
        padding: 20
    }));
         return spriteData.pipe(gulp.dest('last/images'));
});
...
gulp.task('watchlist',function (cb) {
    sequence('clean','jshint',['css','sprite','cssconcat','img','jsconcat','js','html'])(cb)
});
...
gulp.task('default',function (cb) {
    sequence('clean','jshint',['css','img','js','sprite','cssconcat','jsconcat','html','connect'],'watch')(cb)
});
